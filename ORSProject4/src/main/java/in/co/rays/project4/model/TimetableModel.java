...
public class TimetableModel {
	...
	public int add(TimetableBean tb) throws ApplicationException, DuplicateRecordException {
		...
		// TimetableBean bean11 = checkBycds(tb.getCourse_Id(),tb.getSemester(),
		// new
		// java.sql.Date(tb.getExam_Date().getTime()));
		// TimetableBean bean12 = checkBycss(tb.getCourse_Id(),
		// tb.getSubject_Id(),
		// tb.getSemester());
		/*
	...
	public void update(TimetableBean tb) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			// throw new ApplicationException("Exception in update Method of
			// TimeTable Model");
		} finally {
			JDBCDataSource.closeConnection(conn);
		}
